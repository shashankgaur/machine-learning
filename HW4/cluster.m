function cluster(k, data, data_name)

% 1) prepare the data for the algorithms

% 1.1) colors for the graphs
colors = ['c', 'g', 'm', 'b', 'y', 'r'];

% 1.2) total number of data points (both datasets have the same size)
N = size(data, 1);

% 1.3) start the cycle of 5 different initial means
j = 1;
...
while j < 6
    ...
    title(sprintf('K-Means (k = %d dataset : %s)',k, data_name));
    ...
    title(sprintf('GMM after EM (k = %d dataset : %s)',k, data_name));
