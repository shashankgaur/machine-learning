...
private:
	...
	static double dot_p(const svm_node *px, const svm_node *py); 
	...
	double kernel_poly_linear(int i, int j) const
	{
		return powi(gamma*dot_p(x[i],x[j])+coef0, degree) + (dot(x[i],x[j]) - dot_p(x[i],x[j])); //
	}
	double kernel_rbf(int i, int j) const
	{
		return exp(-gamma*(x_square[i]+x_square[j]-2*dot(x[i],x[j])));
	}
	double kernel_rbf_linear(int i, int j) const
	{ //TODO:-improve speed
		return exp(-gamma*(x_square[i]+x_square[j]-2*dot_p(x[i],x[j]))) + (dot(x[i],x[j]) - dot_p(x[i],x[j])); 
	}
 ...
 gamma(param.gamma), coef0(param.coef0)
{
	switch(kernel_type)
	{
		...
		case SIGMOID:
			...
			break;
		case POLY_LINEAR:
			kernel_function = &Kernel::kernel_poly_linear;
			break;
		case RBF_LINEAR:			
			kernel_function = &Kernel::kernel_rbf_linear;
			break;
	}
	...
	if(kernel_type == RBF_LINEAR)
	{
		x_square = new double[l];
		for(int i=0;i<l;i++)
			x_square[i] = dot_p(x[i],x[i]);
	}
}
...
double Kernel::dot_p(const svm_node *px, const svm_node *py)
{	// dot limited to first p dimensions
	double sum = 0;
	int p_counter = 0;
	while(px->index != -1 && py->index != -1 && p_counter < dimension_)
	{
		if(px->index == py->index)
		{
			sum += px->value * py->value;
			  ...
			  const svm_parameter& param)
{
	switch(param.kernel_type)
	{
		...
		case POLY_LINEAR:
			return powi(param.gamma*dot_p(x,y)+param.coef0, param.degree) + (dot(x,y) - dot_p(x,y));
		case RBF:
		{
			double sum = 0;
		...
		case RBF_LINEAR:
		{
			double sum = 0;
			int curr_p = 0;
			while(x->index != -1 && y->index !=-1 && curr_p < dimension_)
			{
				if(x->index == y->index)
				{
					double d = x->value - y->value;
			...
			while(x->index != -1 && curr_p < dimension_)
			{
				sum += x->value * x->value;
				curr_p ++;
				++x;
			...
			while(y->index != -1 && curr_p < dimension_)
			{
				sum += y->value * y->value;
				curr_p ++;
				++y;
			}
			return exp(-param.gamma*sum) + (dot(x,y) - dot_p(x,y));
		}
		case SIGMOID:
...
{
	"linear","polynomial","rbf","sigmoid", "poly_linear", "rbf_linear", NULL
};
